...
class BE_Team:
    def __init__(self, myToken, myApp, pool=False):
        self.lineServer =  "https://gxx.line.naver.jp"
        self.thisHeaders = {}
        splited = myApp.split("\t")
        ...
        if splited[0] == "ANDROIDLITE":
            depis = splited[3].split(".")
            self.thisHeaders["user-agent"] = 'LLA/{} SM-J500G {}'.format(splited[1], splited[3])
        ...
        elif splited[0] in ["IOS"]:
            self.thisHeaders["user-agent"] = 'Line/{} Iphone8 {}'.format(splited[1], splited[3])
        elif splited[0] in ["DESKTOPMAC"]:
            self.thisHeaders["user-agent"] = 'Line/{} DESKTOPMAC {}'.format(splited[1], splited[3])
        else :
    ...
    def gc(self,chatMid,opsi,ticket=None):
        sw = {
            'chatMid':self.getChats([chatMid]).chats[0].chatMid,
            'createdTime':self.getChats([chatMid]).chats[0].createdTime,
            'chatName':self.getChats([chatMid]).chats[0].chatName,
            'picturePath':self.getChats([chatMid]).chats[0].picturePath,
            'creator':self.getChats([chatMid]).chats[0].extra.groupExtra.creator,
            'ticket':self.getChats([chatMid]).chats[0].extra.groupExtra.preventedJoinByTicket,
            'url':self.getChats([chatMid]).chats[0].extra.groupExtra.invitationTicket,
            'members':self.getChats([chatMid]).chats[0].extra.groupExtra.memberMids,
            'invitee':self.getChats([chatMid]).chats[0].extra.groupExtra.inviteeMids,
            'ticketId':self.reissueChatTicket(chatMid).ticketId,
        }
        if opsi == "ticket" and ticket != None:
            x = self.getChats([chatMid]).chats[0]
            if x.extra.groupExtra.preventedJoinByTicket != ticket:
                x.extra.groupExtra.preventedJoinByTicket = ticket
                self.updateChat(x,4)
                re = "QR update"
            else:
                re = "Hmm"
        elif opsi == 'ticketId' and ticket == 'url':
            re = 'https://line.me/R/ti/g/'+self.reissueChatTicket(chatMid).ticketId
        else:
            re = sw.get(opsi)
        return re